...
const registerSchema = z.object({
  ...
  if (d.role === "hr") return !!d.companyName && d.companyName.trim().length > 0;
  return true;
}, {
  message: "Company name is required for HR managers.",
  path: ["companyName"],
}).refine((d) => {
  if (d.role === "hr") return !!d.department && d.department.trim().length > 0;
  return true;
}, {
  ...
  async function onSubmit(values: z.infer<typeof registerSchema>) {
    ...
    try {
      ...
      toast({
        ...
        description: "Welcome to InterVent. You can now sign in.",
      });
      setLocation("/");
    } catch (err: unknown) {
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className={selectedRole === "student" ? "md:col-span-2" : ""}>
                            ...
                            <FormControl>
                              <Input placeholder="+1 (555) 000-0000" {...field} className="h-11" data-testid="input-phone" />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
