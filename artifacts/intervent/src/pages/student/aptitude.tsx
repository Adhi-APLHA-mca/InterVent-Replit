...
import {
  ...
  Eye, Brain, ChevronRight, Lock, ChevronLeft, CheckSquare
} from "lucide-react";
...
const CATEGORY_STYLES: Record<string, { bg: string; text: string; bar: string }> = {
  "Numerical Reasoning":  { bg: "bg-blue-500/15",   text: "text-blue-600 dark:text-blue-400",   bar: "bg-blue-500" },
  "Logical Reasoning":    { bg: "bg-purple-500/15",  text: "text-purple-600 dark:text-purple-400",  bar: "bg-purple-500" },
  "Verbal Ability":       { bg: "bg-teal-500/15",    text: "text-teal-600 dark:text-teal-400",    bar: "bg-teal-500" },
  "Data Interpretation":  { bg: "bg-orange-500/15",  text: "text-orange-600 dark:text-orange-400",  bar: "bg-orange-500" },
  "General Aptitude":     { bg: "bg-green-500/15",   text: "text-green-600 dark:text-green-400",   bar: "bg-green-500" },
};
...
export default function AptitudePage() {
  ...
  const doSubmit = async (forced = false) => {
    ...
    try {
      ...
      const res = await fetch(`${FASTAPI_URL}/api/aptitude/submit`, {
        method: "POST",
      ...
      setLocation(`/student/aptitude/results?job_id=${jobId}&candidate_id=${candidateId}`);
    } catch {
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        const idToken = await user.getIdToken();
        const res = await fetch(`${FASTAPI_URL}/api/aptitude/generate`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        const qs: AptitudeQuestion[] = data.questions;
        setQuestions(qs);
        selectedRef.current = Array(qs.length).fill("");
        setSelected(Array(qs.length).fill(""));
        setPhase("instructions");
      } catch (err) {
        toast({ title: "Could not load aptitude test", description: err instanceof Error ? err.message : "Unknown error", variant: "destructive" });
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          />
          <p className="text-foreground font-medium">Submitting your aptitude test…</p>
        ...
        >
          <div className="bg-gradient-to-r from-[#667eea]/10 to-[#764ba2]/10 border-b border-border px-6 py-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-bold text-foreground text-lg">Stage 2 — Aptitude Test</h1>
                <p className="text-muted-foreground text-xs">InterVent AI-Proctored MCQ Assessment</p>
              </div>
            </div>
          </div>
          ...
          <div className="px-6 py-5 space-y-5">
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { icon: Brain,   label: "15 questions",            sub: "MCQ format" },
            ...
            <div className="bg-muted/60 border border-border rounded-xl p-4 space-y-2">
              <p className="text-foreground font-semibold text-sm">5 categories covered:</p>
              <div className="flex flex-wrap gap-2">
                {["Numerical Reasoning","Logical Reasoning","Verbal Ability","Data Interpretation","General Aptitude"].map((cat) => {
                  const s = CATEGORY_STYLES[cat] ?? { bg: "bg-muted", text: "text-muted-foreground" };
                  return (
                    <span key={cat} className={cn("text-[11px] font-medium px-2.5 py-1 rounded-full", s.bg, s.text)}>
                      {cat}
                    </span>
                  );
                })}
            ...
            <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 space-y-2">
              ...
              <ul className="text-muted-foreground text-xs space-y-1.5">
                {[
                  ...
                  "Select one option per question (A, B, C or D).",
                  "Navigate freely — you can revisit any question.",
            ...
            >
              <Shield size={15} /> Start Aptitude Test
            </Button>
          </div>
        </motion.div>
      </div>
  ...
  const catStyle = CATEGORY_STYLES[cat] ?? { bg: "bg-muted", text: "text-muted-foreground", bar: "bg-muted-foreground" };
    ...
    <div className="min-h-screen bg-background text-foreground select-none flex flex-col">
      ...
      <div className="fixed top-0 left-0 right-0 z-50 bg-card/95 backdrop-blur-sm border-b border-border h-14 flex items-center px-4 lg:px-6 gap-4">
        <div className="flex items-center gap-2 shrink-0">
          ...
          <span className="font-bold text-sm hidden sm:block">InterVent Aptitude</span>
        </div>
        ...
        <div className="flex-1 flex justify-center">
          <motion.div
            ...
            className={cn(
              "flex items-center gap-2 px-4 py-1.5 rounded-full font-mono font-bold text-sm border",
              timeCritical
            ...
            {questions.map((_, i) => (
              <button
                ...
                className={cn(
                  ...
                  i === currentQ
                    ? "bg-gradient-to-br from-[#667eea] to-[#764ba2] text-white scale-110"
                    : selected[i]
            ...
            >
              ...
              <div className="px-5 pt-4 pb-3 border-b border-border">
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  ...
                  <span className={cn("text-[11px] font-semibold px-2.5 py-0.5 rounded-full", catStyle.bg, catStyle.text)}>
                    {cat}
                  </span>
                </div>
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isSelected
                          ? "border-[#667eea] bg-[#667eea]/10 text-foreground"
