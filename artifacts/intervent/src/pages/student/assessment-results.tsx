...
import { XCircle, Clock, Trophy, ArrowLeft, Loader2, CalendarCheck, Code2 } from "lucide-react";
    ...
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        if (!data.assessment || data.assessment.status !== "completed") {
          setLocation(`/student/assessment?job_id=${jobId}&candidate_id=${candidateId}`);
          return;
        }
        setAssessment(data.assessment as AssessmentData);
      } catch {
        ...
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className={cn("px-6 py-6 text-center",
            ...
            <div className="flex items-center justify-center gap-2 mb-2">
              ...
              <span className="text-xs text-muted-foreground font-medium uppercase tracking-wide">Stage 1 — Technical</span>
            </div>
            <p className={cn("text-xl font-bold mb-2",
          ...
          <div className="grid grid-cols-3 divide-x divide-border border-t border-border">
            ...
            <div className="py-4 text-center">
              <p className="text-lg font-bold text-foreground flex items-center justify-center gap-1">
                ...
                {assessment.time_taken_seconds ? formatDuration(assessment.time_taken_seconds) : "—"}
              </p>
              <p className="text-xs text-muted-foreground mt-0.5">Time Taken</p>
            </div>
            <div className="py-4 text-center">
              <p className={cn("text-lg font-bold", (assessment.violations ?? 0) > 0 ? "text-amber-500" : "text-foreground")}>
        ...
        {passed ? (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-blue-700 dark:text-blue-400 text-sm">Stage 1 Complete!</p>
                <p className="text-xs text-muted-foreground mt-0.5">Proceed to Stage 2 — Aptitude Test on your dashboard.</p>
              </div>
            </div>
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
            ...
            <p className="text-sm text-muted-foreground">Keep practising technical concepts and check back for future opportunities.</p>
